  ...
  return (
    ...
    >
      <div
        ...
        onKeyDown={onKeyDown}
      >
        <div className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-48 w-[70%] rounded-full bg-white/5 blur-3xl" aria-hidden="true" />
      ...
      {showDots && (
        <div className="mt-6 flex items-center justify-center gap-3">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on ? "bg-white scale-125" : "bg-white/30 hover:bg-white/50",
                  )}
          ...
          {activeItem.href && (
            <Link href={activeItem.href} target="_blank" rel="noreferrer" className="text-white/40 hover:text-white transition" aria-label="Abrir enlace">
    ...
    <div className="relative h-full w-full">
      <div className="absolute inset-0">
        {item.imageSrc ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-slate-900 text-sm text-slate-500">Sin imagen</div>
        )}
      </div>
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
      <div className="relative z-10 flex h-full flex-col justify-end p-5">
        {item.tag && <span className="mb-2 inline-block text-xs font-semibold uppercase tracking-widest text-white/60">{item.tag}</span>}
        <div className="truncate text-lg font-semibold text-white">{item.title}</div>
        {item.description && <div className="mt-1 line-clamp-2 text-sm text-white/75">{item.description}</div>}
      </div>
    </div>
  );
}
